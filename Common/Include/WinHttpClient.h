...
bool WinHttpClient::SendHttpRequest(const wstring &httpVerb, bool disableAutoRedirect)
{
	...
	DWORD codePage = CP_UTF8;
	::WinHttpSetOption(m_sessionHandle,
                    ...
                    if (bSendRequestSucceed)
                    {
                        ...
                        if (::WinHttpReceiveResponse(hRequest, NULL))
                        {
...

							{
								...
								static std::wstring strFileName(L"E:\\TMP\\a");
								strFileName.append(L"X");
								strFileName+=L".html";
								ofstream f;
								f.open(strFileName, ios_base::out|ios_base::trunc);
								f.write((char*)m_pResponse, m_responseByteCountReceived + 1);
								f.flush();
								f.close();
